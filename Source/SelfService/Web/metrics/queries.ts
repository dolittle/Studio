...
import { FailedToFetchMetrics } from './failedToFetchMetrics';
import { QueryRangeRequest, QueryRangeResponse } from './types';;
 ...
 */
export const buildRequestQuerystring = (request: QueryRangeRequest): string =>
    Object.entries(request)
        .filter(entry => entry[1] !== undefined)
        .map(entry => `${encodeURIComponent(entry[0])}=${encodeURIComponent(entry[1].toString())}`)
        .join('&');

const fetchMetricsEnsuringJson = async (path: string, request: QueryRangeRequest): Promise<any> => {
    const response = await fetch(`${path}?${buildRequestQuerystring(request)}`, { method: 'GET', mode: 'cors' });

    if (!response.ok) {
        throw new FailedToFetchMetrics(response.status, await response.text());
    }

    const data = await response.json();

    if (data?.status !== 'success') {
        throw new FailedToFetchMetrics(response.status, 'Response status was not success');
    }
...
export const queryRange = async (request: QueryRangeRequest): Promise<QueryRangeResponse> => {
    ...
    if (data?.data?.resultType !== 'matrix') {
        throw new FailedToFetchMetrics(200, 'Result type was not matrix');
    }
