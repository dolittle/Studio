...
export const useReconnecting = (url: string, columns: number, rows: number): TerminalStreams => {
    ...
    const connectToServer = useMemo(() =>
        async () => {
            ...
            try {
                const connection = await connect(url, columns, rows);
                connecting = false;
                await clear();
    ...
    return { input: input.writable, output: output.readable };;
};
