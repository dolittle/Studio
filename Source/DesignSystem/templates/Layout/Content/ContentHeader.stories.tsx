...
const { metadata, createStory } = componentStories(ContentHeader, {
    decorator: (story) => <ContentContainer>{story()}</ContentContainer>,
});
...
export const Subtitle = createStory({titleTextVariant: 'subtitle'});
export const WithToolbar = createStory({buttonsSlot: <div><Switch.UI id='switch' label='Switch' value={true}/></div>});
